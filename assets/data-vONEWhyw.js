...
</iframe>`,codeLanguage:`html`,quiz:{question:`iframe의 권한을 제한해 보안을 강화하는 속성은?`,questionEn:`Which attribute restricts an iframe's permissions for security?`,options:[`secure`,`sandbox`,`restrict`,`safe`],optionsEn:[`secure`,`sandbox`,`restrict`,`safe`],correctIndex:1,explanation:`sandbox 속성은 iframe 내부의 스크립트 실행·폼 제출 등 기능을 기본 차단하고, allow-* 토큰으로 필요한 권한만 허용합니다.`,explanationEn:`The sandbox attribute blocks features like script execution and form submission inside the iframe by default, allowing only needed permissions via allow-* tokens.`}}]}],a=[{id:`01-selectors`,chapter:1,titleKey:`css01`,sections:[{title:`CSS란?`,titleEn:`What is CSS?`,content:`CSS(Cascading Style Sheets)는 HTML 문서의 시각적 표현을 정의하는 스타일시트 언어입니다. 색상, 폰트, 레이아웃, 애니메이션 등을 제어합니다. CSS는 세 가지 방법으로 적용할 수 있습니다: 인라인, 내부(style 태그), 외부(.css 파일).`,contentEn:`CSS (Cascading Style Sheets) is a stylesheet language that defines the visual presentation of HTML documents. It controls colors, fonts, layout, animations, and more. CSS can be applied in three ways: inline, internal (style tag), and external (.css file).`,code:`<style>
...
}`,codeLanguage:`jsx`,quiz:{question:`커스텀 Hook의 이름이 따라야 하는 규칙은?`,questionEn:`What naming convention must a custom Hook follow?`,options:[`get으로 시작`,`use로 시작`,`Hook으로 끝남`,`대문자로 시작`],optionsEn:[`Start with get`,`Start with use`,`End with Hook`,`Start with uppercase`],correctIndex:1,explanation:`커스텀 Hook은 반드시 use로 시작해야 React가 Hook 규칙을 적용하고 린터가 올바르게 검사할 수 있습니다.`,explanationEn:`A custom Hook must start with "use" so React applies the Rules of Hooks and the linter can check it correctly.`}}]},{id:`04-context`,chapter:4,titleKey:`react04`,sections:[{title:`Context로 전역 상태 공유`,titleEn:`Sharing Global State with Context`,content:`props를 여러 단계로 내려보내는 prop drilling을 피하려면 Context를 사용합니다. createContext로 컨텍스트를 만들고 Provider로 값을 공급하면, 하위 컴포넌트는 useContext로 어디서든 그 값을 읽을 수 있습니다. 테마·로그인 정보 등 전역 데이터에 적합합니다.`,contentEn:`To avoid prop drilling — passing props down many levels — use Context. Create a context with createContext and supply a value with Provider; descendant components can read it anywhere with useContext. It suits global data like theme and auth info.`,code:`import { createContext, useContext, useState } from 'react';
...
}`,codeLanguage:`typescript`,quiz:{question:`string | number 유니온에서 number 분기로 타입을 좁히는 방법은?`,questionEn:`How do you narrow to the number branch in a string | number union?`,options:[`value === number`,`typeof value === "number"`,`value instanceof Number`,`value as number`],optionsEn:[`value === number`,`typeof value === "number"`,`value instanceof Number`,`value as number`],correctIndex:1,explanation:`typeof value === "number" 검사를 통과한 분기에서는 TypeScript가 value를 number로 좁혀 추론합니다.`,explanationEn:`In the branch passing the typeof value === "number" check, TypeScript narrows value to number.`}}]},{id:`03-advanced-types`,chapter:3,titleKey:`ts03`,sections:[{title:`enum과 리터럴 타입`,titleEn:`Enums and Literal Types`,content:`enum은 이름이 있는 상수 집합을 정의합니다. 다만 번들 크기와 트리셰이킹 측면에서, 유니온 리터럴 타입(as const)이 더 가볍고 권장되는 경우가 많습니다. 두 방식의 차이를 이해하고 상황에 맞게 선택합니다.`,contentEn:`enum defines a named set of constants. However, for bundle size and tree-shaking, union literal types (as const) are often lighter and recommended. Understand the difference and choose appropriately.`,code:`// enum
