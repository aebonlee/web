...
# 마이페이지 진행률 + 관리자 대시보드(회원별 결과) (2026-06-03)

## 요청
로그인 시 마이페이지에서 진행률 확인, 관리자는 대시보드에서 각 회원의 결과 열람.

## 작업 내용
- **마이페이지(Profile)**: "진행률" 섹션 추가 — 전체 진행 바(완료/총 문항 · %) + 난이도 카테고리별(입문/중급/고급/실전) 미니 바.
  진행 데이터는 기존 `web_user_progress`(로그인 시 Supabase 동기화)·로컬을 사용.
- **관리자 대시보드(AdminDashboard)**: 스텁 → 실제 구현.
  - `user_profiles`(web 회원) + `web_user_progress`(전체) 조회 → 회원별 완료수·진행률·카테고리별 집계.
  - 요약 카드(회원 수/학습 시작/평균 진행률/전체 완료) + 회원 결과 표(진행 바·카테고리별).
  - `AdminGuard`로 ADMIN_EMAILS만 접근. RLS 차단 시 안내 메시지 처리.
- **RLS**: `supabase/web_user_progress.sql` 추가 — **테이블 생성(CREATE)** + 본인 read/write + 관리자 전체 read 정책.
  (관리자 대시보드가 전 회원 진행을 보려면 Supabase에서 1회 실행 필요. 미적용 시 본인 데이터만 보임.)
- `site.css`에 진행 바·대시보드 표 스타일(+다크모드) 추가.

## 검증
- `npm run typecheck` ✅ / `npm run build` ✅ / preview /profile·/admin/dashboard 200, CSS 번들 포함.

## 🐛 후속: web_user_progress 테이블 부재
- 증상: RLS SQL 실행 시 `relation "public.web_user_progress" does not exist`. 테이블 자체가 미생성 상태였음
  (그래서 로그인 사용자 진행률 클라우드 동기화가 조용히 실패, 로컬만 동작).
- 해결: `supabase/web_user_progress.sql`을 **CREATE TABLE(복합 PK user_id,problem_id) + 인덱스 + RLS + 정책**
  완전판으로 교체. Supabase SQL Editor에서 1회 실행하면 테이블·보안이 모두 구성됨.
  복합 PK로 두어 ProgressContext의 onConflict 미지정 upsert가 중복 없이 정상 동작.

## 🐛 Progress 페이지 상단 커뮤니티 통계 (RLS 영향)
- 증상: web_user_progress에 RLS(본인+관리자)를 켠 뒤, Progress 상단 "커뮤니티 현황"이
  일반 사용자에겐 본인 데이터만(또는 anon 0) 보임 — 집계가 비어 보임.
- 원인: 상단 통계가 raw 행을 직접 select → RLS로 타인 행 비가시.
- 해결: 집계 전용 `web_progress_stats()` RPC(SECURITY DEFINER, 개인정보 미반환) 추가,
  Progress.tsx가 `supabase.rpc('web_progress_stats')`로 조회. `supabase/web_progress_stats.sql` 1회 실행 필요.
  (RPC 미생성 시 통계 영역은 안내 메시지로 안전하게 degrade.)

## 🐛 page-header 타이틀 하단 여백 (학습현황 등)
- 증상: /progress 등 page-header의 타이틀 아래 여백이 없어 부제/콘텐츠와 붙어 보임.
- 원인: `.page-header`가 `h2`만 스타일링했는데 페이지들은 `<h1>` 사용 → 전역 `*{margin:0}`로 h1 margin-bottom=0,
  폰트 크기도 기본값.
- 해결: `.page-header h1, .page-header h2` 공통 규칙으로 통합(font-size 36px·margin-bottom 12px·line-height 1.25).
  Progress·Profile·TopicDetail·About 등 page-header 사용 페이지 전체 타이틀 여백/크기 일관화.

## 전 화면 상단 여백 점검 (2026-06-03)
- page-header 사용(네비 오프셋 내장): Problems·ProblemsByCategory·ProblemDetail·Playground·Guide·Progress·Profile·About·TopicDetail → h1 여백 수정으로 일관화 ✓
- Auth(Login·Register·ForgotPassword) `.auth-fullpage`: `calc(nav-height+40px)` ✓
- Home(hero)·TopicList(topic-hero): 자체 네비 오프셋 ✓
- NotFound: 네비 오프셋 없어 콘텐츠가 고정 네비에 걸침 → `padding-top: calc(nav-height+40px)` + `min-height: calc(100vh - nav-height)`로 수정 ✓

## 전 화면 재점검 결과 (2026-06-03)
- typecheck/build ✅, preview 33개 라우트 전부 200(빌드·라우팅 정상).
- 라이브 딥링크는 GitHub Pages SPA 패턴상 HTTP 404 상태이나 `public/404.html`(pathSegmentsToKeep=0)이
  리다이렉트해 브라우저에서 정상 로딩 — 동작 확인.
- 발견·수정: 404.html `<title>`이 'DreamIT Books'(템플릿 잔재) → 'DreamIT Web | 드림아이티 웹 개발 학습'으로 정정.

## 🐛 마이페이지/관리자 대시보드 접근 불가 (네비 링크 오류)
- 증상: 기능은 구현됐으나 사용자 메뉴 링크가 잘못된 경로라 도달 불가.
  - "마이페이지" → `/mypage`(미존재, 실제는 `/profile`) → 404
  - 주문내역 → `/mypage/orders`(미존재, shop 비활성)
  - 관리자 → 부모 사이트 `www.dreamitbiz.com/admin`(이 사이트의 `/admin/dashboard` 아님)
- 해결: Navbar 사용자 드롭다운 링크 정정 — 마이페이지→`/profile`, 주문→학습현황 `/progress`,
  관리자→내부 `/admin/dashboard`(isAdmin만). 클릭 시 메뉴 닫힘 처리 추가.
